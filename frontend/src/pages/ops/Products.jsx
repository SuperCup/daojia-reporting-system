    ...
    <div className="page-container">
      ...
      <Modal title={modal.editing ? '编辑商品' : '新增商品'} open={modal.open} onOk={handleSave} onCancel={() => { setModal({ open: false, editing: null }); form.resetFields() }} width={600}>
        <Form form={form} layout="vertical">
          <Form.Item name="upc" label="UPC条码" rules={[{ required: true }]}>
            <Input disabled={!!modal.editing} />
          </Form.Item>
          <Form.Item name="name" label="产品名称" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="brand_series" label="品牌系列"><Input /></Form.Item>
          <Form.Item name="center_series" label="中心系列"><Input /></Form.Item>
          <Form.Item name="detail_series" label="明细系列"><Input /></Form.Item>
          <Form.Item name="spec" label="单瓶规格(ml)"><Input /></Form.Item>
          <Form.Item name="pack_quantity" label="装箱数"><Input type="number" /></Form.Item>
